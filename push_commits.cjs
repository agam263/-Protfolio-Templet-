...
for (let i = 0; i < messages.length; i++) {
  console.log(`Commit ${i + 1}/87: ${messages[i]}`);
  try {
    fs.appendFileSync('CHANGELOG.md', `- ${messages[i]}\n`);
    execSync(`git add CHANGELOG.md`);
    execSync(`git commit -m "${messages[i]}"`);
    execSync(`git push -f origin main`);
  } catch (e) {
    console.error(`Failed to push commit ${i + 1}`);
  }
}
console.log('All 87 commits pushed successfully!');
